...
import json
import csv
from metrics.pycocotools.coco import COCO
from metrics.pycocoevalcap.eval import COCOEvalCap

"""Captioning evaluation functions below"""
...
def evaluate_metrics_from_lists(predictions: List[str],
    ...
    # TODO Check the below statements as its only applicable for Clotho
...
def evaluate_metrics(prediction_file: Union[str, Path, List[Dict[str, str]]],
    ...
    for row in prediction_file:
        ...
        try: 
            ...
            #print(f"Issue with parsing: {reference_dict[file_name]}")
            cap_names = ['caption_{:1d}'.format(i) for i in range(1, len(reference_dict[file_name]))]
            ground_truths.append([reference_dict[file_name][cap] for cap in cap_names])

        #print(reference_dict[file_name])
        #ground_truths.append([reference_dict[file_name][cap] for cap in cap_names])
...
def parse_output_for_diffmetrics(predicted_output, ref_captions, file_names=None):
    captions_pred, captions_gt = [], []
    
    for i, (pred_cap, ref_cap) in enumerate(zip(predicted_output, ref_captions)):
        captions_pred.append({'file_name': i, 'caption_predicted': pred_cap})
        captions_gt.append({'file_name': i, 'caption_1': ref_cap})

    return captions_pred, captions_gt

def parse_output_for_capmetrics(predicted_output, ref_captions, file_names):
    captions_pred, captions_gt = [], []

    count = 1
    fname_old = None
    for i, (pred_cap, ref_cap, fname) in enumerate(zip(predicted_output, ref_captions, file_names)):
        if fname_old == fname:
            count += 1
        else:
            fname_old = fname
        ...
        if count == 1:
            captions_pred.append({'file_name': fname, 'caption_predicted': pred_cap})
            captions_gt.append({'file_name': fname, f'caption_{count}': ref_cap})
        else:
            captions_gt[-1].update({f'caption_{count}': ref_cap})
